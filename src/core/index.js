...
const { __ARABIC__, __KHMER__, __LAO__, __MODERN__, __PERSIAN__, __ROMAIN__, __THAI__, __ERROR_ROMAIN__ } = require('../constants');
...
const convert = (number, typeResultNumerals) => {
  ...
  if ((typeResultNumerals === __ROMAIN__) && (detecteNumeralTypeOfInput(number) !== __ROMAIN__)) {
    return ((+number < 1) || (+number > 3999)
      ?
      new Error(__ERROR_ROMAIN__).message
      :
    ...
    switch (typeResultNumerals) {
      ...
      case __KHMER__: {
        return convert(RomainToDecimal(number), __KHMER__);
      }
      case __LAO__: {
        return convert(RomainToDecimal(number), __LAO__);
      }
      case __MODERN__: {
        return convert(RomainToDecimal(number), __MODERN__);
      }
      case __PERSIAN__: {
        return convert(RomainToDecimal(number), __PERSIAN__);
      }
      case __THAI__: {
        return convert(RomainToDecimal(number), __THAI__);
      }
      default: {
        return number;
      }
